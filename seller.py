...
class Seller_Win:
    def __init__(self, root):
        ...
        #title
        lbl_title=Label(self.root,text="ADD SELLER DETAILS",font=("times new roman",29,"bold"),bg="black",fg="gold",bd=4,relief=RIDGE)
        ...
        #label frame----
        labelframeleft=LabelFrame(self.root,bd=2,relief=RIDGE,text="Seller Details",font=("times new roman",10,"bold"), padx=2)
        labelframeleft.place(x=5,y=50,width=425,height=490)

        #labels and entry
        lbl_cust_ref=Label(labelframeleft,text="Seller Ref",font=("arial",10,"bold"),padx=2,pady=6)
        lbl_cust_ref.grid(row=0,column=0,sticky=W)
        entry_ref=ttk.Entry(labelframeleft,width=22,font=("arial",10,"bold"),textvariable=self.var_ref,state="readonly")
        entry_ref.grid(row=0,column=1)

        #cust name
        cname=Label(labelframeleft,text="Seller name:",font=("arial",10,"bold"),padx=2,pady=6)
        ...
        btn_frame.place(x=0,y=420,width=512,height=30)
        
        btn_customer = Button(labelframeleft, text="Add Property Details",font=("arial",10,"bold"),bg="black",fg="gold", command=self.open_property_page)
        btn_customer.grid(row=11, column=0, padx=3)
    ...
    def add_data(self):
        if self.var_mobile.get()=="" or self.var_mother.get()=="":
            ...
            try:
                ...
                my_cursor.execute("INSERT INTO seller VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
    (self.var_ref.get(), self.var_cust_name.get(), self.var_mother.get(), self.var_gender.get(),
     ...
     self.var_idproof.get(), self.var_idnumber.get(), self.var_address.get())
                ...
                messagebox.showinfo("Success","Seller has been added",parent=self.root)
                conn.commit()
                self.fetch_data()
    ...
    def fetch_data(self):
        conn=mysql.connector.connect(host="localhost",username="root",password="SLMedha21!",database="mydata")
        my_cursor=conn.cursor()
        my_cursor.execute("select * from seller")
    ...
    def update(self):
        if self.var_mobile.get()=="":
            ...
            conn=mysql.connector.connect(host="localhost",username="root",password="SLMedha21!",database="mydata")
            my_cursor=conn.cursor()
            my_cursor.execute("Update seller set  Name=%s,mother=%s,gender=%s,postcode=%s,mobile=%s,email=%s,nationality=%s,idproof=%s,idnumber=%s,address=%s where ref=%s",
            ...
            self.fetch_data()
            conn.close()
            messagebox.showinfo("Update","Seller details has been updated sucessfully",parent=self.root)
    def mDelete(self):
        mDelete=messagebox.askyesno("Real Estate", "Do you want to delete this seller",parent=self.root)
        if mDelete>0:
            conn=mysql.connector.connect(host="localhost",username="root",password="SLMedha21!",database="mydata")
            my_cursor=conn.cursor()
            query="delete from seller where Ref=%s"
            value=(self.var_ref.get(),)
            my_cursor.execute(query,value)
            messagebox.showinfo("Delete","Seller's detail has been deleted sucessfully",parent=self.root)
        else:
            if not mDelete:
              return
        conn.commit()
    ...
    def search(self):
        conn=mysql.connector.connect(host="localhost",username="root",password="SLMedha21!",database="mydata")
        my_cursor=conn.cursor()
        my_cursor.execute("SELECT * FROM seller WHERE " + str(self.search_var.get()) + " LIKE '%" + str(self.txt_search.get()) + "%'")
        rows=my_cursor.fetchall()
        if len( rows)!=0:
            self.Cust_Details_Table.delete(*self.Cust_Details_Table.get_children())
            for i in rows:
