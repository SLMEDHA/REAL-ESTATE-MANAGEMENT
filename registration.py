...
class Register_Win:
    def __init__(self, root):
        ...
        regnum.grid(row=6,column=0,sticky=W)
        txt_regnum=ttk.Entry(labelframeleft,width=22,font=("arial",10,"bold"),textvariable=self.var_regnum,)
        txt_regnum.grid(row=6,column=1)

        #registration name
        regname=Label(labelframeleft,text="Registration name:",font=("arial",10,"bold"),padx=2,pady=6)
        regname.grid(row=7,column=0,sticky=W)
        txt_regname=ttk.Entry(labelframeleft,width=22,font=("arial",10,"bold"),textvariable=self.var_regname)
        txt_regname.grid(row=7,column=1)

        #buyer ID
        regname=Label(labelframeleft,text="Buyer ID:",font=("arial",10,"bold"),padx=2,pady=6)
        regname.grid(row=8,column=0,sticky=W)
        txt_regname=ttk.Entry(labelframeleft,width=22,font=("arial",10,"bold"),textvariable=self.var_buyer)
        txt_regname.grid(row=8,column=1)

        #registration id
        regid=Label(labelframeleft,text="Registration ID:",font=("arial",10,"bold"),padx=2,pady=6)
        regid.grid(row=9,column=0,sticky=W)
        txt_price=ttk.Entry(labelframeleft,width=22,font=("arial",10,"bold"),textvariable=self.var_regid)
        txt_price.grid(row=9,column=1)
       
        #buttons
        btn_frame=Frame(labelframeleft,bd=2,relief=RIDGE,bg="pink")
        ...
        #show data table
        details_table=Frame(table_frame,bd=2,relief=RIDGE,bg="pink")
    ...
    def add_data(self):
        if self.var_address.get()=="" or self.var_price.get()=="" :
            ...
            try:
                ...
                my_cursor.execute("INSERT INTO registration (reg_id, reg_num, buyer_id, seller_id, reg_name, reg_type, reg_status) VALUES (%s, %s, %s, %s, %s, %s, %s)",
                (self.var_regid.get(), self.var_regnum.get(), self.var_buyer.get(), self.var_seller.get(),
                self.var_regname.get(), self.var_type.get(), 'pending'))
    ...
    def mDelete(self):
        ...
        if mDelete>0:
            conn=mysql.connector.connect(host="localhost",username="root",password="SLMedha21!",database="mydata")
            my_cursor=conn.cursor()
            query="delete from registration where reg_id=%s"
