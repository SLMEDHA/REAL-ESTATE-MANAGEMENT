...
class Property_Win:
    def __init__(self, root):
        ...
        #title
        lbl_title=Label(self.root,text="ADD PROPERTY DETAILS",font=("times new roman",29,"bold"),bg="black",fg="gold",bd=4,relief=RIDGE)
    ...
    def add_data(self):
        if self.var_address.get()=="" or self.var_price.get()=="" :
            ...
            try:
                ...
                my_cursor.execute("INSERT INTO property (propid, pseller, ptype, pdesc, paddr, price) VALUES (%s, %s, %s, %s, %s, %s)",
    (self.var_propid.get(), self.var_seller.get(), self.var_type.get(), self.var_desc.get(),
    self.var_address.get(), self.var_price.get()))
                messagebox.showinfo("Success","Property has been added",parent=self.root)
                conn.commit()
                self.fetch_data()
    ...
    def fetch_data(self):
        conn=mysql.connector.connect(host="localhost",username="root",password="SLMedha21!",database="mydata")
        my_cursor=conn.cursor()
        my_cursor.execute("select * from property WHERE pstatus='ready'")
    ...
    def update(self):
        if self.var_address.get()=="" or self.var_price.get()=="" :
            ...
            conn=mysql.connector.connect(host="localhost",username="root",password="SLMedha21!",database="mydata")
            my_cursor=conn.cursor()
            my_cursor.execute("UPDATE property SET pseller=%s, ptype=%s, pdesc=%s, paddr=%s, price=%s WHERE propid=%s",
            (self.var_seller.get(), self.var_type.get(), self.var_desc.get(),
            self.var_address.get(), self.var_price.get(), self.var_propid.get())
            )

            conn.commit()
            self.fetch_data()
            conn.close()
            messagebox.showinfo("Update","Property details has been updated sucessfully",parent=self.root)
    def mDelete(self):
        mDelete=messagebox.askyesno("Real Estate", "Do you want to delete this Property",parent=self.root)
        if mDelete>0:
            conn=mysql.connector.connect(host="localhost",username="root",password="SLMedha21!",database="mydata")
            my_cursor=conn.cursor()
            query="delete from property where propid=%s"
